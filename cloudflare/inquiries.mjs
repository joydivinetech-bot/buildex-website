...
function notificationMessage(data,id){
 ...
 const descriptionLabel=data.kind==='estimate'?'Project description':'Message';
 const text=[title,'','Reference: '+id,...fields.map(([label,value])=>label+': '+value),...(data.details.description?['',descriptionLabel,data.details.description]:[])].join('\n');
 ...
 const reply=data.email?'mailto:'+encodeURIComponent(data.email):'',call='tel:'+data.phone.replace(/[^\d+]/g,'');
 const description=data.details.description?`<div style="margin-top:22px;color:#64748b;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.7px">${descriptionLabel}</div><div style="margin-top:8px;padding:15px;background:#f7f9fb;border-left:3px solid #c79a43;color:#263b50;font-size:14px;line-height:1.6;white-space:pre-wrap">${escapeHtml(data.details.description)}</div>`:'';
 const replyButton=reply?`<a href="${reply}" style="display:inline-block;background:#0b2948;color:#ffffff;text-decoration:none;font-size:14px;font-weight:700;padding:11px 18px;border-radius:5px;margin-right:8px">Reply by email</a>`:'';
 const html=`<!doctype html><html><body style="margin:0;background:#f3f6f9;font-family:Arial,sans-serif;color:#102a43"><div style="display:none;max-height:0;overflow:hidden">${escapeHtml(title)} from ${escapeHtml(data.name)}</div><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f3f6f9;padding:24px 12px"><tr><td align="center"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;background:#ffffff;border:1px solid #dce5ee;border-radius:8px;overflow:hidden"><tr><td style="background:#0b2948;padding:24px 28px;border-bottom:4px solid #c79a43"><div style="color:#ffffff;font-size:22px;font-weight:700">Buildex Construction</div><div style="color:#cbd5e1;font-size:13px;margin-top:5px">${escapeHtml(title)}</div></td></tr><tr><td style="padding:24px 28px"><div style="display:inline-block;background:#eef3f8;color:#0b2948;font-size:11px;font-weight:700;letter-spacing:1px;padding:6px 9px;border-radius:4px;margin-bottom:18px">${escapeHtml(data.kind.toUpperCase())}</div><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border:1px solid #e1e8ef;border-radius:6px;border-collapse:separate;border-spacing:0;overflow:hidden">${rows}</table>${description}<div style="margin-top:22px">${replyButton}<a href="${call}" style="display:inline-block;background:#ffffff;color:#0b2948;text-decoration:none;font-size:14px;font-weight:700;padding:10px 18px;border:1px solid #9fb1c3;border-radius:5px">Call customer</a></div></td></tr><tr><td style="padding:16px 28px;background:#f7f9fb;color:#718096;font-size:11px">Reference: ${escapeHtml(id)} · Submitted through buildexconstructions.com</td></tr></table></td></tr></table></body></html>`;
...
export async function handleInquiry(request,env,ctx={},fetcher=fetch){
 ...
 try{
  ...
  const data={name:field(d,'name',150,true),email:field(d,'email',200),phone:field(d,'phone',30,true),service:field(d,'service',200,true),kind:field(d,'kind',20,true),details:{}};
  if((data.email&&!email(data.email))||data.phone.replace(/\D/g,'').length<7||!['contact','estimate'].includes(data.kind))reject(400,'Check your contact details.');
  for(const [key,max] of Object.entries({description:5000,address:500,propertyType:200,size:200,startDate:200,budget:200}))data.details[key]=field(d,key,max);
  if(data.kind==='estimate'&&!['Residential','Commercial'].includes(data.details.propertyType))reject(400,'Include the property type.');
