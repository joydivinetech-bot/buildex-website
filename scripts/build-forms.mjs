...
await mkdir(path.join(out,'admin'),{recursive:true});for(const [source,target] of [['inquiry-dashboard.html','dashboard.html'],['inquiry-dashboard.js','dashboard.js'],['inquiry-dashboard.css','dashboard.css']])await copyFile(path.join(root,'public',source),path.join(out,'admin',target));
await writeFile(path.join(out,'_headers'),"/*\n  X-Content-Type-Options: nosniff\n  Referrer-Policy: strict-origin-when-cross-origin\n  X-Frame-Options: DENY\n  Content-Security-Policy: default-src 'self'; img-src 'self' https://images.pexels.com data:; script-src 'self' https://challenges.cloudflare.com; frame-src https://challenges.cloudflare.com; connect-src 'self' https://challenges.cloudflare.com; style-src 'self' 'unsafe-inline'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'\n");
