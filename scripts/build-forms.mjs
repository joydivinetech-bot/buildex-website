...
const pipeline=(await readFile(path.join(root,'cloudflare/inquiries.mjs'),'utf8')).replaceAll('export async function','async function');const worker=(await readFile(path.join(root,'cloudflare/forms-worker.mjs'),'utf8')).replace("import {handleInquiry} from './inquiries.mjs';",'');await writeFile(path.join(out,'_worker.js'),pipeline+'\n'+worker);
